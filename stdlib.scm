...
(define fold-left
	(let ((null? null?)
				...
				(car car)
				(cdr cdr))
				(letrec ((fold-left-loop (lambda (proc acc lst)
				(if (null? lst)
				acc
				(fold-left-loop proc (proc acc (car lst)) (cdr lst))
				))))
				fold-left-loop
				)
				)
...
(define fold-right
	(let ((null? null?)
				...
				(car car)
				(cdr cdr))
				(letrec ((fold-right-loop (lambda (proc acc lst)
				(if (null? lst)
				acc
				(proc (car lst) (fold-right-loop proc acc (cdr lst)))
				))))
				fold-right-loop
				)
				)
...
(define reverse_lst
	(let ((null? null?)
				...
				(letrec ((reverse_lst_loop (lambda (lst)
				(if (null? lst)
				'()
				(append (reverse_lst_loop (cdr lst)) (list (car lst)))
				)
				)
				))
				reverse_lst_loop
...
(define cons*
	(let ((null? null?)
				...
				(if (null? y)
				x
				(fold-right cons (car (reverse_lst y)) (cons x (reverse_lst (cdr (reverse_lst y)))))
				)			
	)))
